...
- **Character voice samples:** each of the 8 recurring characters has a short reference voice-over clip (served as static audio, `*_voice_sample_url` in the API); a "listen" button next to each character in the theme picker and the episode summary plays it in place, independent of theme selection. See [docs/ses-rehberi.md](docs/ses-rehberi.md) for the character-to-voice mapping and [docs/karakter-ve-mekan-incili.md](docs/karakter-ve-mekan-incili.md) for the full cast/location/theme reference.
...
| `GET /episodes/themes` | List all 28 fixed Neşeli Orman themes, with cast/location names, reference images, and voice sample URLs. |
...
| `POST /episodes/generate-batch` | Generate every one of the 28 themes' episodes in one call; optional `project_id` (requires a bearer token for that project's owner). Themes the project already has an episode for are skipped, not re-generated, so it's safe to call again. |
